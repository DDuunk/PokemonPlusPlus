/**
 * @file		GameEnitity.cpp
 * @date		17-09-2019
 * @author		Dylan Duunk
 *
 * @brief		The base class for all game entities, providing position, rotation, and scaling functionality to all entities.
 *
...
public:
	/**
	 * @brief		Enum to describe either world space or local space, used to get Position/Rotation/Scale associated with each space
	 */
	enum SPACE { local = 0, world = 1 };

private:
	// Position/Rotation/Scale/Active/Parent set to private to make they are not set directly, 
	// but the functions are used instead   
	Vector2 mPos;
	float mRotation;
	Vector2 mScale;
...
public:
	/**
	 * @brief 		pos is the starting position of the new GameEntity
     */
	GameEntity(Vector2 pos = VEC2_ZERO);
	~GameEntity();

	/**
		 * @brief 		Sets the GameEntity's position, it only updates local space
		 *
	...
	/**
	 * @brief		Returns the position of the GameEntity
	 *
	...
	/**
	 * @brief		Sets the GameEntity's rotation, it only updates local space
	 *
	...
	/**
	 * @brief		Returns the rotation of the GameEntity
	 *
	...
	/**
	 * @brief		Sets the GameEntity's scale, it only updates local space
	 */
	void Scale(Vector2 scale);
	/**
	 * @brief		Returns the scale of the GameEntity
	 *
	...
	/**
	 * @brief		Sets the active state of the GameEntity
	 */
	void Active(bool active);
	/**
	 * @brief		Returns the active state of the GameEntity
	 */
	bool Active();

	/**
	 * @brief		Sets the parent of the GameEnity as the given GameEntity
	 *
	...
	/**
	 * @brief		Returns the Current parent of the GameEntity,
	 *				Returns NULL if the GameEntity has no parent
	 */
	GameEntity* Parent();

	/**
	 * @brief		Translates the GameEntity's lcoal position by the given amount
	 */
	void Translate(Vector2 vec);
	/**
	 * @brief		Rotates the GameEntity's locally by the given amount
	 */
